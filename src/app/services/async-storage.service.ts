...
async function getByCountry(entityType: string, entityCountry: string): Promise<Clock> {
    const entities = await query(entityType)
    const entity = entities.find(entity => entity.country === entityCountry)
    if (!entity) throw new Error(`Cannot get, Item ${entityCountry} of type: ${entityType} does not exist`)    
    return entity;
}
